...
```assembly
   0x080485a4 <+0>:     push   ebp                  # Push Base Pointer
   0x080485a5 <+1>:     mov    ebp,esp              # Set ebp to esp
   0x080485a7 <+3>:     and    esp,0xfffffff0       # Align stack
   0x080485aa <+6>:     sub    esp,0x40             # Allocate 64 bytes 
   0x080485ad <+9>:     lea    eax,[esp+0x16]       # Set eax to esp+22 
   0x080485b1 <+13>:    mov    DWORD PTR [esp],eax  # Set [esp] to eax ()
   0x080485b4 <+16>:    call   0x804851e <pp>       # Call pp
   0x080485b9 <+21>:    lea    eax,[esp+0x16]       # Set eax to esp+22
   0x080485bd <+25>:    mov    DWORD PTR [esp],eax  # Set esp to eax
   0x080485c0 <+28>:    call   0x80483b0 <puts@plt> # Call puts
   0x080485c5 <+33>:    mov    eax,0x0              # Set eax on 0
   0x080485ca <+38>:    leave  
   0x080485cb <+39>:    ret 
```
...
```assembly
   0x0804851e <+0>:     push   ebp                              # Push Base Pointer
   0x0804851f <+1>:     mov    ebp,esp                          # Set ebp to esp
   0x08048521 <+3>:     push   edi                              # Push Index register
   0x08048522 <+4>:     push   ebx                              # Push General register
   0x08048523 <+5>:     sub    esp,0x50                         # Allocate 80 bytes
   0x08048526 <+8>:     mov    DWORD PTR [esp+0x4],0x80486a0    # Set esp+4 value to " - "
   0x0804852e <+16>:    lea    eax,[ebp-0x30]                   # Set eax to ebp-48
   0x08048531 <+19>:    mov    DWORD PTR [esp],eax              # Set esp to eax
   0x08048534 <+22>:    call   0x80484b4 <p>                    # Call p
   0x08048539 <+27>:    mov    DWORD PTR [esp+0x4],0x80486a0    # Set esp+4 value to " - "
   0x08048541 <+35>:    lea    eax,[ebp-0x1c]                   # Set eax to ebp+28
   0x08048544 <+38>:    mov    DWORD PTR [esp],eax              # Set esp to eax (ebp+28)
   0x08048547 <+41>:    call   0x80484b4 <p>                    # Call p
   0x0804854c <+46>:    lea    eax,[ebp-0x30]                   # Set eax to ebp-48 
   0x0804854f <+49>:    mov    DWORD PTR [esp+0x4],eax          # Set esp+4 to eax
   0x08048553 <+53>:    mov    eax,DWORD PTR [ebp+0x8]          # Set eax to ebp+8
   0x08048556 <+56>:    mov    DWORD PTR [esp],eax              # Set esp to eax(ebp+8)
   0x08048559 <+59>:    call   0x80483a0 <strcpy@plt>           # Call strcpy
   0x0804855e <+64>:    mov    ebx,0x80486a4                    # Set ebx to 32 (0x20)
   0x08048563 <+69>:    mov    eax,DWORD PTR [ebp+0x8]          # Set eax to ebp+8
   0x08048566 <+72>:    mov    DWORD PTR [ebp-0x3c],0xffffffff  # Set ebp-60 value to 0xffffffff (4294967295 in 64-bit, -1 in 32-bit)
   ...
   0x08048579 <+91>:    repnz scas al,BYTE PTR es:[edi]         # Repeat string operation 
   0x0804857b <+93>:    mov    eax,ecx                          # Set eax to ecx (result of repnz scas)
   0x0804857d <+95>:    not    eax                              # Inverse eax bits (eax *= -1 -1) 
   0x0804857f <+97>:    sub    eax,0x1                          # Remove the last byte
   0x08048582 <+100>:   add    eax,DWORD PTR [ebp+0x8]          # Set eax to ebp+8 value
   0x08048585 <+103>:   movzx  edx,WORD PTR [ebx]               # Set edx to ebx value
